...
def trainPPO(player, enemy, env, save_interval, load, T_horizon, Max_train_steps): # PPO using XinJingHao's implementation
    episode_length = 500

    
    if load and os.path.exists("model/model.pth"):
        print("Loading model")
        player.load(torch.load("weights/model.pth", weights_only=True))
        enemy.load(torch.load("weights/model.pth", weights_only=True))
