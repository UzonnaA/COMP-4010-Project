...
class PPO_discrete():
    ...
    def save(self, episode):
        torch.save(self.critic.state_dict(), f"./model/{self.agent_name}_critic_{episode}.pth")
        torch.save(self.actor.state_dict(), f"./model/{self.agent_name}_actor_{episode}.pth")

    def load(self, episode):
        self.critic.load_state_dict(torch.load(f"./model/{self.agent_name}_critic_{episode}.pth", map_location=self.dvc))
        self.actor.load_state_dict(torch.load(f"./model/{self.agent_name}_actor_{episode}.pth", map_location=self.dvc))
